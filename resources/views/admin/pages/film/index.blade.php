          ...
          <table class="table align-items-center mb-0">
            <thead>
              <tr>
                ...
                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Launch at</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="align-middle text-center">
                  <a href="{{ route('admin.film.edit', $data->id) }}" class="text-secondary font-weight-bold text-xs me-5" data-toggle="tooltip" data-original-title="Edit user">
                    Edit
                  </a>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <input type="file" class="form-control" placeholder="Email" aria-label="Email" aria-describedby="email-addon" name="thumbnail" required>
            @error('thumbnail')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            <input type="text" class="form-control" placeholder="Title" aria-label="Title" aria-describedby="title-addon" id="title" name="title" required>
            @error('title')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            <input type="text" class="form-control" placeholder="Slug (automation)" aria-label="Slug" aria-describedby="slug-addon" id="slug" name="slug" readonly>
            @error('slug')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            <input type="number" class="form-control" placeholder="Total Minute" aria-label="Name" aria-describedby="email-addon" name="total_minute" required>
            @error('total_minute')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            <input type="text" class="form-control" placeholder="URL vidio, type embeded" aria-label="Path Source Vidio" aria-describedby="email-addon" name="path_src_vidio" required>
            @error('path_src_vidio')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          <div class="mb-3">
            <textarea type="text" class="form-control" placeholder="Description ..." aria-label="Password" aria-describedby="password-addon" name="description"></textarea>
            @error('password')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
