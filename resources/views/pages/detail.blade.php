...
<script>
    document.getElementById('shorten-button').addEventListener('click', async () => {
        let customCode = document.getElementById('custom-code').value.trim(); // Mengambil nilai dari input dan menghapus spasi
        ...
        try {
            const response = await fetch('/short-url', {
                ...
                body: JSON.stringify({ code: customCode, url: currentUrl })
            });
